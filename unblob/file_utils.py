...
def carve(carve_path: Path, file: File, start_offset: int, size: int, mode:int = 0o644):
...
class FileSystem:
    ...
    def carve(self, path: Path, file: File, start_offset: int, size: int, mode:int = 0o644):
    ...
    def create_symlink(self, src: Path, dst: Path):
        ...
        if safe_link:
            ...
            logger.warning(f"Creating symlink {src} -> {dst}")
