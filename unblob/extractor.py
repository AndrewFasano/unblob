...
def make_symlinks_relative(base_dir):
    '''
    Given a base directory, we ensure all symlinks are relative
    '''
...
def fix_extracted_directory(outdir: Path, task_result: TaskResult):
    '''
    Given an extracted directly, we ensure all symlinks are
    relative and point to files within the extraction directory
    '''

    if not outdir.exists():
        return

    make_symlinks_relative(outdir)
